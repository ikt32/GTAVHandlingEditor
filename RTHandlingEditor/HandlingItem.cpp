...
RTHE::CHandlingDataItem RTHE::ParseXMLItem(const std::string& sourceFile) {
    ...
    if (shdNode) {
        auto localLog = [&](const std::string& arg) {
            logger.Write(DEBUG, fmt::format("[Parse] [{}] {}", fileName, arg));
        };

        localLog("Parsing SubHandlingData");
        ...
        while (shdItemElement != nullptr) {
            ...
            if (std::string("CCarHandlingData") == type) {
                localLog("Parsing SubHandlingData.Item<CCarHandlingData>");
                ...
                if (advancedDataNode != nullptr) {
                    localLog("Parsing SubHandlingData.Item<CCarHandlingData>.AdvancedData");
                ...
                localLog(fmt::format("Unknown SubHandlingData.Item<{}>. Adding dummy item.", type_));
...
bool RTHE::SaveXMLItem(const CHandlingDataItem& handlingDataItem, const std::string& targetFile) {
    ...
    auto localLog = [&](const std::string& arg) {
        logger.Write(DEBUG, fmt::format("[Write] [{}] {}", fileName, arg));
    };

    if (handlingDataItem.subHandlingData.size() == 0) {
        localLog("No SubHandlingData");
        InsertElementSubHandlingNULL(itemNode);
    }
    else {
        localLog(fmt::format("Found {} SubHandlingData entries", handlingDataItem.subHandlingData.size()));
        ...
        for (size_t i = 0; i < handlingDataItem.subHandlingData.size(); ++i) {
            ...
            localLog(fmt::format("Found SubHandlingData.Item[{}]<{}>", i, type));
            ...
            if (type == RTHE::HANDLING_TYPE_CAR) {
                ...
                if (carHandlingData.pAdvancedData.size() > 0) {
                    localLog(fmt::format("Found SubHandlingData.Item[{}]<CCarHandlingData>.AdvancedData", i));
                    ...
                    for (uint32_t i = 0; i < carHandlingData.pAdvancedData.size(); ++i) {
                        ...
                        localLog(fmt::format("Inserted SubHandlingData.Item[{}]<CCarHandlingData>.AdvancedData", i));
                    }
                ...
                subHandlingDataItem->SetAttribute("type", "NULL");
                shdElement->InsertEndChild(subHandlingDataItem);
                localLog("Inserted SubHandlingData.Item<CCarHandlingData>");
            }
        }
